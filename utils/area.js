...
function getProviceList(){
    ...
    for(let i = 0;i < _lists.length;i++){
      _p.push({ id: _lists[i].id, name: _lists[i].name, has_children: _lists[i].has_children, ad_name: _lists[i].ad_name })
    }
    return _p;
}
      ...
      if(pdata.has_children){
        ...
        for (let j = 0; j < clen;j++){
          if (c.indexOf(plist[j].name) != -1) {
            let myp = { "pid": _lists[i].id, "name": plist[j].name, "id": plist[j].id, "ad_name": plist[j].ad_name}
